...
class LevenshteinIndex(IndexTemplate):
            ...
            for _ in range(max_attempts):
                ...
                with ThreadPoolExecutor(max_workers=self.n_jobs) as executor:
                    results = list(executor.map(
                        lambda x: self._compute_distance_decision(x, s1, s2),
                        [strings[idx] for idx in indices]
                    ))
    ...
    def get_nns_by_vector(self, query_str, topk=None, include_distances=False):
        ...
        # with ThreadPoolExecutor(max_workers=self.n_jobs) as executor:
        #     all_matches = list(executor.map(lambda tree: self._query(tree, query_str), self.trees))
        all_matches = []
